...
export default async function handler(req, res) {
  ...
  // ✅ Required: email, deviceId, signature
  ...
  if (!Array.isArray(queryResult) || !queryResult[0]?.document) {
    return res.status(200).json({ allow: false, message: 'User not found' });
  }

  const docFields = queryResult[0].document.fields || {};
  if (
    docFields.deviceId?.stringValue !== deviceId ||
    docFields.signature?.stringValue !== signature
  ) {
    return res.status(200).json({ allow: false, message: 'Device ID or Signature mismatch' });
  }
